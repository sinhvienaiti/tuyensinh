<?php
	...
	if(isset($_POST['edit_thongbao'])){
		...
		$content = addslashes($_POST['content']);
		$img = $_FILES['img']['name'];
		$update = "UPDATE thongbao SET title = '$title',img='$img',content = '$content' WHERE id =".$g_id;
		$query_up = mysqli_query($con,$update);
			if($query_up){
				$_SESSION['request'] = "Sửa thành công";
	...
	if(isset($_POST['edit_tintucts'])){
		...
		$update = "UPDATE tintuc SET  title = '$title',content = '$content',timeup = '$time',img = '$img',file = '$file', description = '$description' WHERE id =".$g_id;
		$query_up = mysqli_query($con,$update);
			if($query_up){
				$_SESSION['request'] = "Sửa thành công";
