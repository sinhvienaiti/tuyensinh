		...
		<div style="width: 1018px; margin: 0px auto;">
			...
			<div  style="width: 74%;float: left; background:#fff; margin:10px 0px 0px 0px; padding-bottom: 100px;">
				...
				<div style="margin: 25px; font-weight: bold;font-size: 16px;">
					...
					Với mong muốn giải đáp những thắc mắc có liên quan đến tuyển sinh 2017, trường ĐHDL Duy Tân mở trang Tư vấn trực tuyến. Quý vị phụ huynh và các bạn thí sinh có thể đặt câu hỏi tại đây hoặc gửi email về địa chỉ: <a href="" title=""> tuyensinh@dtu.edu.vn</a>. <p>Chúng tôi sẽ cố gắng hồi âm trong thời gian ngắn nhất.</p>
				
				</div>
				<div>
					<?php
					require_once('../php/code_view_question.php');
					?>
				</div>
				 <div class="col-md-12">
					...
					<?php if($p > 1){ ?>
						<li style="cursor: pointer;">
							<a title="Previous" href="<?php echo $localhost; ?>/html/form_view_question.php?id_views=<?php echo $prew; ?>">
									Previous
							</a>
						</li>
					...
					<?php }
					 for($i=1;$i<=$all_table;$i++){ 
						...
						<li style="cursor: pointer;" class="<?php if($p == $i){echo 'active';} ?>">
							<a title="<?php echo $i; ?>" href="<?php echo $localhost; ?>/html/form_view_question.php?id_views=<?php echo $i; ?>">
								<?php echo $i; ?>
							</a>
						</li>
					<?php }
					...
					if($p < $all_table){ ?>
						<li style="cursor: pointer;">
							<a title="Next" style="cursor: pointer;" href="<?php echo $localhost; ?>/html/form_view_question.php?id_views=<?php echo $next; ?>">
								Next
							</a>
						</li>
						<?php } ?>
