...
<body style="text-transform: capitalize;font-family:'Open Sans',sans-serif,arial,helvetica,verdana" onload="unchatclick()">
	...
	<?php
	 ...
	 ?>
	<h1 class="ribbon" style="border-bottom-right-radius: 2em;border: 1px solid #fff;"><?php echo $row_nn['name']; ?></h1>
	<?php } } 
	
			$select_tt = "SELECT * FROM in_tintuc_ct WHERE ma_tt = 1 LIMIT 0,5";
	...
	<p class="rib" >
		...
		<a style="letter-spacing: 1px;" href="http://localhost/tuyensinh/html/form_tintuc_ct.php?ma_tt=<?php echo $i; ?>&id_tt=<?php echo $row_tt['id'] ?>" title="<?php echo $row_nn['title']; ?>"> 
		- <?php echo $row_tt['title']; ?>
		</a>
		<?php if($row_tt['new_tt'] == 1){ ?>
		<img src="../img/quangcao/logo/icon-new.gif" alt="picture">
		<?php } ?>
	</p>

	<?php } } ?>
	<p class="rib red">
		 <a style="font-size: 20px;float: right; margin-top: 10px;" href="http://localhost/tuyensinh/html/form_tintuc_title.php?ma_tt=<?php echo $i; ?>" title="xem thêm">xem thêm...</a>
	</p>
	</div>
	<?php } ?>
