	...
	<div class="container">
		<div style="width: 1018px; margin: 0px auto;">
			...
			<div  style="width: 74%;float: left; background:#fff; margin:10px 0px 0px 0px; padding-bottom: 100px;">
				...
				<?php 
				$id_question = isset($_GET['id_question']) ? (int)$_GET['id_question'] : false;
				if($id_question == false){
						die('
								...
								<h1 style="font-style: italic; font-weight: bold; margin: 20px 20px 20px 20px; color: #114682; border-bottom: 2px solid red; padding-bottom: 20px;">
								 ERROR !!! NOT FORUM
								 <a class="btn btn-primary" href="http://localhost/tuyensinh/html/form_view_question.php">quay lại</a>
							</h1>
							</div>
						...
						$select_nn = "SELECT * FROM make_question WHERE id = $id_question";
						$query_nn = mysqli_query($con,$select_nn);
						if(mysqli_num_rows($query_nn) > 0){
							$row_nn = mysqli_fetch_assoc($query_nn);
							?>
							<div style="margin: 20px;">
								<h1 style="margin: 20px; font-style: italic; color: red;">
								<img src="../img/quangcao/logo/langmanager.png" alt="picture">	hỏi: 

								</h1>
								
									<p style="margin: 20px; margin-left: 70px; font-size: 17px;">
									-	<?php echo $row_nn['title']; ?> 
									</p>
									<p style="float: right; margin-bottom: 30px;">
										<span style="font-style: italic;">(<?php echo $row_nn['timeup']; ?>)</span>
									</p>
							</div>
							<div style="margin: 50px; border: 1px solid #0081ca; border-radius: 15px;">
								<h1 style="margin: 20px; font-style: italic; color: #84841f;">
								<img style="width: 48px; height: 48px;" src="../img/quangcao/logo/group_edit.png" alt="picture">	trả lời:

								</h1>
								
									<p style="margin: 20px; margin-left: 70px; font-size: 17px;font-weight: bold;">
									-	<?php echo $row_nn['nd']; ?>
									</p>
							</div>
							<?php
				...
				?>
				<div style="margin: 25px; font-weight: bold;font-size: 16px;">
					
					<a class="btn btn-primary"  href="<?php echo $localhost; ?>/html/form_view_question.php" title="qua trang câu hỏi">
						qua trang câu hỏi
					</a>
					<a class="btn btn-success" style="float: right;" href="<?php echo $localhost; ?>/html/form_make_question.php" title="đặt câu hỏi">
						đặt câu hỏi
					</a>
				</div>
